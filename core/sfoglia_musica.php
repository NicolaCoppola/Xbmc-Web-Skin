...
if ($send == "1" ){
	...
	foreach ($results1 as $value){ 
		...
		$img_thumb_cambia  = 'core/img/musica/Song.png';
	...
	<div id="info_file">
					...
					<div id="title"   >
					<a href="?media=music&send='.($send-1).'&directory='.$back_folder.'" 
					onMouseOver="window.parent.document.getElementById(\'img_cambia\').src=(\'core/img/back.png\');" 
					onMouseOut="window.parent.document.getElementById(\'img_cambia\').src=(\'core/img/xbmc.png\');">&ensp;&ensp;&ensp; '.$t_indietro.'
					
					</a></div> 
					
					<div id="playlist"><a href="?media=music&send='.($send-1).'&directory='.$back_folder.'"></a></div>
					<div id="type"    ><a href="?media=music&send='.($send-1).'&directory='.$back_folder.'"><img src="img/back.png" width="28" height="28"/></a></div>
		...
		$img_thumb_cambia  = 'core/img/musica/Folder.png';
			...
			echo ' 
					<div id="info_file">
							...
							<div id="title"   ><a href="?media=music&send='.($send+1).'&directory='.$value['file'].'&back='.$_GET['directory'].'" onMouseOver="window.parent.document.getElementById(\'img_cambia\').src=(';
							...
							echo');">&ensp;&ensp;&ensp; '.$value['label'].'</a></div> 
							<div id="playlist"><a href="?media=music&send='.($send+1).'&directory='.$value['file'].'&back='.$_GET['directory'].'"></a></div>
							<div id="type"    ><a href="?media=music&send='.($send+1).'&directory='.$value['file'].'&back='.$_GET['directory'].'">';
